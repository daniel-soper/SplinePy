...
import os
import pandas as pd
from spline_t_cdf import t_cdf
...
def run_benchmark():
    """
    Performs a rigorous benchmark of the spline t-CDF vs. SciPy's t-CDF.
    """
    print("--- Verifying T-Distribution Spline Accuracy and Performance ---")
    num_test_points = 10_000_000
    num_trials = 30
    
    all_trial_results = []

    # Use a realistic range for degrees of freedom in testing
    df_range = (1, 100000)
    x_range = (-10000, 10000)
    ...
    for trial_num in range(1, num_trials + 1):
        ...
        scipy_cdf_vals = np.array([t.cdf(x, df) for x, df in zip(test_x_vals, test_df_vals)])
        end_scipy = time.time()
        ...
        spline_cdf_vals = np.array([t_cdf(x, df) for x, df in zip(test_x_vals, test_df_vals)])
    ...
    results_path = 'splines/t/results_spline_t_cdf.xlsx'
